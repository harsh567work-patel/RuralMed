...
const DB_PATH = process.env.DB_PATH || path.join(__dirname, 'ruralmed.db');
...
export function initDatabase() {
  ...
  db.serialize(() => {
    ...
    // Create indexes for better query performance (Phase 5)
    ...
    // Seed default PHC inventory items (§8.7) — INSERT OR IGNORE is safe on existing DBs
    const seedItems = [
...
/**
 * Seed demo data for showcasing system functionality (§8.7)
...
export async function seedDemoData() {
  try {
    ...
    // Demo patients with varied vitals for alert demonstration
    const demoPatients = [
